...
func (o *Psr2) ExecuteDetailed(c config.Config) ([]LintResult, error) {
	cmd := exec.Command(
		"docker", "run", "--rm",
		"-v", c.Path+":/app",
	...
	results := parseFixerJsonOutput(stdout.Bytes(), stderr.Bytes(), o.Slug(), "PSR-2", c.Path)

	if len(results) > 0 {
		return results, fmt.Errorf("PSR-2 violations found")
	}

	return results, nil
}

func (o *Psr2) Execute(c config.Config) (string, error) {
